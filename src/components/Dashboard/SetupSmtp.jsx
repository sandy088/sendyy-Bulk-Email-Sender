...
export const SetupSmtp = () => {
    ...
    const checkSMTP = async () => {
        ...
        await axios.post('https://bulk-email-sender-backend.onrender.com/api/v2/smtp-check', data, {
            ...
            console.log("smtp fetched succesfuuly")
        }).catch((error) => console.log("Error occured while fetching smtp details: ", error))
    }
    useEffect(() => {
        checkSMTP()
    }, [])
    ...
    return (
        <div className=''>
            ...
            <div className='flex flex-wrap gap-3 justify-evenly w-11/12 mx-auto mt-6'>


                <div className='rounded-2xl bg-slate-800 flex flex-col gap-3 bg-opacity-80 p-6 w-[90%] md:w-[40%] overflow-hidden'>
                    <form>
                        <label className='text-white flex flex-col gap-2'>
                            ...
                            <input
                                type='text'
                                name='host'
                                placeholder='google.smtp'
                                onChange={onChanngeHandler}
                                className='bg-slate-700 bg-opacity-50 rounded-lg p-2'
                            />
                        </label>
